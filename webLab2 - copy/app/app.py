...
def number_processing(number):
    ...
    if count != 10 and count != 11:
        return make_response(
            render_template('phone.html', flag=0, msg="1 Недопустимый ввод. Неверное количество цифр."))
    if count == 11:
        if not number.startswith('+7') and not number.startswith('8'):
            return make_response(
                render_template('phone.html', flag=0, msg="2 Недопустимый ввод. Неверное количество цифр."))
    number = number.replace('+', '').replace('(', '').replace(')', '').replace('-', '').replace('.', '').replace(' ',
                                                                                                                 '')
    try:
        int(number)
    except ValueError:
        return make_response(render_template('phone.html', flag=0,
